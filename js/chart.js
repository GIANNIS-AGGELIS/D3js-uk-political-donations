...
function updateHistoryBar(d, imagePath) {
    var imageOfDonor = new Image(50, 50);
    imageOfDonor.src = imagePath;
    imageOfDonor.style.margin = "3px";
    imageOfDonor.style.border = "2px solid black";
    imageOfDonor.style.borderRadius = "4px";
    
    imageOfDonor.onmouseover = function (event) {
        var Xcor = event.clientX;
        var Ycor = event.clientY;
        imageOfDonor.style.boxShadow = "0 0 2px 1px rgba(50, 140, 186, 0.7)";
       Tooltip
            .style("opacity", 0.9)
            .html(d.donor)
            .style("left", Xcor-170 +"px")
            .style("top", Ycor +"px");
       
    };
    imageOfDonor.onmouseout = function () {
        imageOfDonor.style.boxShadow = "";
       Tooltip.style("opacity", 0);
        
    };
    newImgElement.appendChild(imageOfDonor);

    if (Counter >= sizeOfHistoryBar) {
        historyElement.removeChild(historyElement.childNodes[sizeOfHistoryBar - 1]); //remove last image
    } else {
        Counter = Counter + 1;
    }
    historyElement.insertBefore(imageOfDonor, historyElement.childNodes[0]); //append new image
}
...
function mouseMoveArc(d) {
    if (d.parent.donor != "root") {
        return chart2Tooltip
            .style("top", (d3.event.Ycor - 90) + "px")
            .style("left", (d3.event.Xcor + 40) + "px");
    }
    else {
        return null;
    }
